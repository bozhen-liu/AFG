...
void run_taint_tests(AFGTestFramework& framework) {
    ...
    // Test 1: Taint configuration loading with detailed validation
    {
        ...
        framework.assert_true(basic_result.visited_functions == origin_result.visited_functions, 
        ...
        framework.assert_true(origin_result.call_graph_nodes >= basic_result.call_graph_nodes, 
        ...
        std::cout << "    Origin: " << origin_result.call_graph_nodes << " nodes, " 
                  << origin_result.points_to_nodes << " points-to" << std::endl;
    }
    ...
    // Test 2: Origin context tracking with different K values
    {
        ...
        framework.assert_true(origin_k2.call_graph_nodes >= origin_k1.call_graph_nodes, 
                            "K=2 should be at least as precise as K=1 for origin tracking");
        framework.assert_true(origin_k3.call_graph_nodes >= origin_k2.call_graph_nodes, 
                            "K=3 should be at least as precise as K=2 for origin tracking");
        
        std::cout << "  Origin Context Precision Progression:" << std::endl;
        std::cout << "    K=1: " << origin_k1.call_graph_nodes << " nodes" << std::endl;
        std::cout << "    K=2: " << origin_k2.call_graph_nodes << " nodes" << std::endl;
        std::cout << "    K=3: " << origin_k3.call_graph_nodes << " nodes" << std::endl;
    }
    ...
    // Test 3: Taint propagation through pointers with correctness validation
    {
        ...
        framework.assert_true(origin_result.points_to_nodes >= basic_result.points_to_nodes, 
        ...
        framework.assert_true(basic_result.visited_functions == origin_result.visited_functions, 
                            "Both analyses should visit the same functions in propagation test");
    ...
    // Test 4: Multiple tainted objects with cross-analysis comparison
    {
        ...
        framework.assert_true(basic_result.visited_functions == kcs_result.visited_functions &&
                            kcs_result.visited_functions == origin_result.visited_functions, 
                            "All analyses should visit the same functions");
        ...
        std::cout << "    Basic: " << basic_result.call_graph_nodes << " nodes" << std::endl;
        std::cout << "    K-callsite: " << kcs_result.call_graph_nodes << " nodes" << std::endl;
        std::cout << "    Origin: " << origin_result.call_graph_nodes << " nodes" << std::endl;
    }
    ...
    // Test 5: Taint through function parameters with context sensitivity
    {
        ...
        framework.assert_true(basic_result.call_graph_edges > 0, "Should track function calls with parameters");
        framework.assert_true(origin_result.call_graph_edges >= basic_result.call_graph_edges, 
    ...
    // Test 6: Context-sensitive taint analysis with detailed validation
    {
        ...
        framework.assert_true(origin_k4.call_graph_nodes >= origin_k2.call_graph_nodes, 
        ...
        framework.assert_true(origin_k1.visited_functions == origin_k2.visited_functions &&
                            origin_k2.visited_functions == origin_k4.visited_functions, 
                            "All K values should visit the same functions");
        
        // Print detailed context sensitivity comparison
        std::cout << "  Context-Sensitive Taint Precision:" << std::endl;
        std::cout << "    K=1: " << origin_k1.call_graph_nodes << " nodes, " 
                  << origin_k1.points_to_nodes << " points-to" << std::endl;
        std::cout << "    K=2: " << origin_k2.call_graph_nodes << " nodes, " 
                  << origin_k2.points_to_nodes << " points-to" << std::endl;
        std::cout << "    K=4: " << origin_k4.call_graph_nodes << " nodes, " 
                  << origin_k4.points_to_nodes << " points-to" << std::endl;
