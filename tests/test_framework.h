...
public:
    ...
    {
        std::string abs_path = std::string(PROJECT_SOURCE_DIR) + "/tests/" + ir_file; // Use PROJECT_SOURCE_DIR for absolute path
        auto module = loadModule(abs_path);
        if (!module)
        {
        ...
        {
            PA = std::make_unique<KCallsitePointerAnalysis>(k_value, *module);
        }
        else if (analysis_mode == "origin")
        {
            PA = std::make_unique<OriginPointerAnalysis>(k_value, *module);
        }
        else
        {
            PA = std::make_unique<PointerAnalysis>(*module);
        }
    ...
    void assert_equals(const uint64_t &expected, const uint64_t &actual, const std::string &message)
    {
        if (expected == actual)
        {
            std::cout << "  ✓ " << message << std::endl;
